...
<Note type="warning">
The `sync:` configuration block shown below is **not yet wired** into the Argus config schema. The underlying `ConfigWatcher` works and is production-ready, but the YAML fields described here have no effect yet. Use `POST /manage/v1/reload` for manual hot-reload in the meantime.
</Note>
