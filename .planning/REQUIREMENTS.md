...
- [ ] **CAT-EXP-01**: 20+ new YAML catalog configs across all existing categories, prioritizing stdio container variants that demonstrate complex patterns (volume mounts, network modes, `system_deps`, env injection with secrets)
...
- [ ] **CAT-EXP-07**: All catalog entries that require API keys or tokens use `${SECRET_NAME}` syntax; at least one complex entry includes an inline comment or YAML example showing how to configure Argus secrets management for that secret
